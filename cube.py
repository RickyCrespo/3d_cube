...
writer = imageio.get_writer(output_file, fps=framerate / 2, codec='libx264', output_params=['-preset', 'slow', '-crf', '18', '-pix_fmt', 'yuv420p'])
...
def on_draw():
    ...
    # Capture the window content to create a frame
    buffer = pyglet.image.get_buffer_manager().get_color_buffer()
    image_data = buffer.get_image_data()
    pixels = np.frombuffer(image_data.get_data('RGBA', -image_data.width * 4), dtype=np.uint8)
    frame = pixels.reshape((image_data.height, image_data.width, 4))
